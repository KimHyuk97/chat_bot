...
# API 키 및 프로젝트 설정
load_dotenv()
logging.langsmith("xlsx Agent 챗봇")

# Streamlit 앱 설정
st.title("xlsx 데이터 분석 챗봇 💬")
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader(
        "xlsx 파일을 업로드 해주세요.", type=["xlsx"], accept_multiple_files=True
    )  # xlsx 파일 업로드 기능
    ...
    if tool_name := tool.get("tool"):
        if tool_name == "python_repl_ast":
            ...
            if query:
                ...
                st.error(
                    "데이터프레임이 정의되지 않았습니다. xlsx 파일을 먼저 업로드해주세요."
                )
                return
...
if apply_btn and uploaded_file:
    # print(uploaded_file)
    loaded_data = pd.read_excel(uploaded_file[0])  # xlsx 파일 로드
